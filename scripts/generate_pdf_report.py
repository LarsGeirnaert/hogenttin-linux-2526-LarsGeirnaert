...
# Voeg gemiddelde temperatuur per dag toe aan weekday_stats
weekday_temp = df.groupby(df["timestamp"].dt.day_name())["temperature"].mean().reindex(
    ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
)
weekday_stats["Gemiddelde temperatuur"] = weekday_temp.values.round(2)
weekday_stats["Gemiddelde"] = weekday_stats["Gemiddelde"].round().astype(int)
...
elements.append(Spacer(1, 12))
table_data = [["Weekdag", "Min", "Max", "Gemiddelde", "Gemiddelde temperatuur"]]
for _, row in weekday_stats.iterrows():
    table_data.append([
        ...
        f"{row['Gemiddelde temperatuur']:.2f}"
    ])
